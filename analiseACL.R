...
nome <- "antonio carlos" #nome do cliente

#encontra documentos que citam o Nome

docsnome <- docst[unique(c(grep(docst$docs, pattern = nome), grep(docst$docs, pattern = "lovato"))), ] #nome e sobrenome
...
for (i in docsnome$docs){
  ...
  num <- length(unique(c(grep(termos[[1]], pattern = "antonio carlos"), 
                         #grep(termos[[1]], pattern = "lovato"), 
                         grep(termos[[1]], pattern = "lovato")))) 
  num3 <- length(unique(c(grep(termos[[1]], pattern = "bortolozzo"), grep(termos[[1]], pattern = "machado")) )) 
  print(num)
...
frasesnome <- frasesnome[-grep(frasesnome, pattern = "bortolozzo")]#exclui frases sobre o SEGUNDO orlando
frasesnome <- frasesnome[-grep(frasesnome, pattern = "machado")]
...
barplot(height = assocs$value[1:26], 
        ...
        las = 1, 
        xlim = c(0:1),
        col = heat.colors(26, alpha=0.7), main = "Palavras Associadas a Antônio Carlos Lovato", 
...
qplot(polaridades, 
      ...
      col=I("Black"),
      binwidth = 1,
      fill=factor(polaridades))+xlab("Score") + ylab("Frequência") + ggtitle("Polaridades por Citações - Antônio Carlos Lovato")

qplot(factor(Classificação), 
      data=polaridades, geom="bar",
      fill=factor(Classificação))+xlab("Categorias") + ylab("Frequência") + ggtitle("Polaridades por Citações - Antônio Carlos Lovato")
